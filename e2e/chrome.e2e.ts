...
test("top bar shows text and survives a double-click", async ({ page }) => {
	...
	await expect(title).toHaveText("Ccez Studio");
	// Browser build has no shell zoom (Tauri-only no-op): the strip
	// stays put and keeps its text.
...
/** Idle timeout is configurable in settings (2–60s). */
test("idle timeout slider persists", async ({ page }) => {
	await openWithMessages(page, [{ role: "user", content: "hi" }]);
	await openSettings(page);
	const slider = page.locator('.settings-panel input[aria-label="Idle seconds before the prompt hides"]');
...
/** Clicking the slider label resets to default (chat width). */
test("clicking the chat-width label text resets to default", async ({ page }) => {
	await openWithMessages(page, [{ role: "user", content: "hi" }]);
	await openSettings(page);
	const slider = page.locator('.settings-panel input[aria-label="Chat width in rem"]');
	await slider.fill("60");
	await expect(slider).toHaveValue("60");
	const label = page.locator(".settings-panel label", { has: slider });
	const box = await label.boundingBox();
	expect(box).toBeTruthy();
	// Top-left of the label is the label text row, clear of the
	// slider, readout, and reset button.
...
test("empty-state language buttons sit clear of the hero", async ({ page }) => {
	...
	// Mac desktop additionally lifts the row (data-mac); elsewhere it
	// simply rests in flow — either way, no overlap.
});
